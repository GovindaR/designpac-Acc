...
<div class="main p20" style="min-height: calc(100vh - 80px);">
    <div class="container">
    <div class="dc-wrap">
        <div class="avt">
        ...
        <form role="form" method="POST" action="{{url('/profile/update')."/".\Illuminate\Support\Facades\Auth::user()->id}}" class="req-form">
            ...
            <div class="form-group">
                ...
                <input type="text" id="organization_name" name="organization_name" value="{{ \App\Package::where('user_id',\Illuminate\Support\Facades\Auth::user()->id)->value('organization_name')}}" style="text-align:center;">
            </div>
            <div class="form-group">
                <label for="position_name">Position/ Job Title:</label><br>
                <input type="text" id="position_name" name="position_name" value="{{ \App\Package::where('user_id',\Illuminate\Support\Facades\Auth::user()->id)->value('position_name')}}" style="text-align:center;">
            </div>
            <div class="form-group">
                <h2><?php
	...
	?></h2>
		<p style="margin-top: -10px;"><i>Contact the authorities to change your package.</i></p>
                {{--<select name="suitable_package" class="form-control">--}}
                    {{--<option value="{{ \App\Package::where('user_id',\Illuminate\Support\Facades\Auth::user()->id)->value('suitable_package')}}" selected>{{ \App\Package::where('user_id',\Illuminate\Support\Facades\Auth::user()->id)->value('suitable_package')}}</option>--}}
